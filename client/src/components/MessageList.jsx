...
export default function MessageList({ conversations, onSelect, selectedId }) {
  if (!conversations?.length) return <div className="p-4 text-gray-500">No conversations yet.</div>;
  return (
    <>
      ...
      {conversations.map(c => (
        <div
          key={c._id}
          onClick={() => onSelect(c)}
          className={`p-4 cursor-pointer ${c._id === selectedId ? 'bg-blue-50' : 'hover:bg-gray-100'}`}
        >
          <div className="font-semibold">{c.senderName}</div>
